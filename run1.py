"""
兼容多种摄像头输入源的版本：
1. 网络摄像头流（WSL中使用，需要在Windows中先运行 camera_server_windows.py）
2. JetCam CSI摄像头（Jetson设备上使用）
3. OpenCV VideoCapture（本地摄像头设备）
"""
import argparse
import sys
import cv2
...
parser = argparse.ArgumentParser(
    description='detect_video from multiple camera sources')
...
parser.add_argument('--test_device', default="cuda:0", type=str,
                    help='cuda:0 or cpu')
# 摄像头输入源选择参数
parser.add_argument('--on_board', default=False, action='store_true',
                    help='使用JetCam CSI摄像头（Jetson设备）')
parser.add_argument('--use_cv2_camera', default=False, action='store_true',
                    help='使用OpenCV VideoCapture直接读取本地摄像头（优先级高于网络流）')
parser.add_argument('--camera_url', default="", type=str,
                    help='URL of camera stream (leave empty to auto-detect Windows IP, 仅在非jetcam和非cv2模式下使用)')
parser.add_argument('--camera_port', default=8081, type=int,
                    help='Port of camera server (default: 8081, 仅在网络流模式下使用)')
parser.add_argument('--width', default=640, type=int,
                    help='摄像头宽度（用于jetcam和cv2模式）')
parser.add_argument('--height', default=360, type=int,
                    help='摄像头高度（用于jetcam和cv2模式）')
...
label_path = "./checkpoints/age-labels.txt"
...
if test_device.startswith("cuda") and not torch.cuda.is_available():
    print(f"Warning: CUDA not available, switching to CPU. (Requested: {test_device})")
    test_device = "cpu"
...
if net_type == 'slim':
    ...
    model_path = "./checkpoints/RFB-AGE3.pth"
    # model_path = "./checkpoints/RFB-Emotion2.pth"
...
# ==================== 摄像头输入源初始化 ====================
# 根据参数选择输入源：jetcam > cv2 > 网络流
camera_source = None
camera_type = None  # 'jetcam', 'cv2', 'network'

if args.on_board:
    # 使用JetCam CSI摄像头（Jetson设备）
    # try:
    from jetcam.csi_camera import CSICamera
    print("初始化JetCam CSI摄像头...")
    # camera_source = CSICamera(
    #     capture_device=0,
    #     width=args.width,
    #     height=args.height,
    ...
    # )
    camera_source = CSICamera(capture_device=0, width=args.width, height=args.height)
    # 使用异步模式：在后台线程中持续读取最新帧
    print("启动JetCam异步读取模式...")
    camera_source.running = True
    camera_type = 'jetcam'
    print(f"使用JetCam CSI摄像头 (异步模式)")
    print(f"分辨率: {args.width}x{args.height}")
    # except ImportError:
    #     print("错误: 无法导入 jetcam 模块")
    #     print("请确保已安装 jetcam: pip install jetcam")
    #     sys.exit(1)
    # except Exception as e:
    #     print(f"错误: 无法初始化JetCam摄像头: {e}")
    #     sys.exit(1)
elif args.use_cv2_camera:
    # 使用OpenCV VideoCapture直接读取本地摄像头
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        # 如果默认设备失败，尝试其他设备
        print("警告: 无法打开摄像头设备 /dev/video0，尝试其他设备...")
        for device_id in range(1, 5):
            cap = cv2.VideoCapture(device_id)
            if cap.isOpened():
                print(f"成功打开摄像头设备 /dev/video{device_id}")
                break
        else:
    ...
    print(f"连接到网络摄像头流: {camera_url}")
    print("如果连接失败，请手动指定Windows IP地址：")
    print(f"  python run1.py --camera_url http://<Windows_IP>:{args.camera_port}/video")
...
def read_frame():
    ...
    if camera_type == 'jetcam':
        # JetCam异步模式：直接获取最新帧（非阻塞）
        frame = camera_source.value.copy() if camera_source.value is not None else None
        return frame
        ...
        try:
            while True:
                chunk = camera_source.raw.read(1024)
                if not chunk:
                    return None
                network_buffer += chunk
                
                # 查找JPEG开始和结束标记
                a = network_buffer.find(b'\xff\xd8')  # JPEG开始标记
                b = network_buffer.find(b'\xff\xd9')  # JPEG结束标记
                
                if a != -1 and b != -1:
                    # 找到完整的JPEG帧
                    jpg = network_buffer[a:b+2]
                    network_buffer = network_buffer[b+2:]  # 保留剩余数据
                    # 解码图像
...
while True:
    try:
        ...
        # 转换为RGB格式（jetcam和cv2返回BGR，网络流已经是BGR）
...
# ==================== 清理资源 ====================
if camera_type == 'jetcam':
    camera_source.running = False  # 停止异步读取
elif camera_type == 'cv2':
